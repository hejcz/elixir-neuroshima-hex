defmodule PubsubtestWeb.CartographersChannel do
  use Phoenix.Channel

  def join("room:cartographers:" <> game_id, _message, socket) do
  ...
  def handle_info(:after_join, socket) do
    broadcast!(
      socket,
      "event_joined",
      Cartographers.Game.representation(socket.assigns[:game_pid])
    )
  ...
  def handle_in("start", _params, socket) do
    if Cartographers.Game.start(socket.assigns[:game_pid], socket.assigns[:player_id]) == :ok do
  ...
  def handle_in("quit", _params, socket) do
    if Cartographers.Game.quit(socket.assigns[:game_pid], socket.assigns[:player_id]) == :ok do
  ...
  def handle_in("act", _params = %{"name" => "draw", "position" => position}, socket) do
    drawing_result =
      Cartographers.Game.draw(socket.assigns[:game_pid], socket.assigns[:player_id], position)
