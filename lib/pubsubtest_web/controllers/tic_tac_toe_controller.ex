defmodule PubsubtestWeb.TicTacToeController do
  ...
  def index(conn, %{"game_id" => game_id}) do
    ...
    if game_pid == nil or not Process.alive?(game_pid) or
      ...
      case TicTacToe.Game.join(game_pid, player_id) do
        :ok ->
          token =
          ...
          conn
          |> PubsubtestWeb.SessionHelper.assign_game_id(game_id, :tic_tac_toe)
          |> render("tic_tac_toe.html", game_id: game_id, token: token)
